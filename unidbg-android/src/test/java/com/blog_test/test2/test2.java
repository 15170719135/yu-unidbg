...
public class test2 extends AbstractJni{
    ...
    test2() {
        ...
        vm = emulator.createDalvikVM(new File("D:\\hecai_pan\\apk\\test\\使用unidbg跑通calculateS\\sinaInternational.apk"));
        //
//        vm = emulator.createDalvikVM(null);

        // 加载目标SO
        DalvikModule dm = vm.loadLibrary(new File("unidbg-android/src/test/java/com/blog_test/test2/libutility.so"), true); // 加载so到虚拟内存
        //获取本SO模块的句柄,后续需要用它
        module = dm.getModule();
